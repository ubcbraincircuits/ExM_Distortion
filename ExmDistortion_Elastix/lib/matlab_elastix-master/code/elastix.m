function varargout=elastix(movingImage,fixedImage,outputDir,paramFile,elastix_path,varargin)
...
[~,elastix_version] = system([elastix_path,'elastix --version']); %Changed by Abhijit
...
if nargin==1 & ischar(movingImage)
    if regexp(movingImage,'^\w')
        ...
        [~,msg]=system([elastix_path,'elastix --',movingImage]); % Changes by Abhijit
    end
    fprintf(msg)
    if nargout>0
...
[status,result]=system([elastix_path, CMD]); % Changes by Abhijit
...
if status %Things failed. Oh dear.
    ...
    if nargout>1
        ...
        d=dir(fullfile(outputDir,'result*.mhd')); % Changed by Abhijit
        if isempty(d)
