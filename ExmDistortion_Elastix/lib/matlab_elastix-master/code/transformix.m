function varargout=transformix(movingImage,parameters,SaveDir,verbose)
...
%% Changes by Abhijit
transformix_path = 'export PATH=/Users/abhijitc/Documents/Software/elastix-5.0.0-mac/bin ; ';
%%

[s,transformix_version] = system([transformix_path,'transformix --version']);
...
[status,result]=system([transformix_path,CMD]);
...
if status %Things failed. Oh dear. 
    ...
    if size(movingImage,2)>3 & ~isempty(movingImage)
        d=dir(fullfile(outputDir,'result.mhd')); %Allow for MHD or TIFF result image
